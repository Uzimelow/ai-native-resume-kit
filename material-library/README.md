...
| `rewrites` | 累积改写：每次 JD 定制完成后，改写版自动存回。下次投同类型岗位时优先复用，微调即可 |
| `notes` | AI 备注：适配弹性、改写建议、注意事项 |

每条改写记录结构：
```
{ id: "rw-001", targetRole: "AI产品运营", targetJD: "百度-AI产品实习生",
  date: "2026-05-24", outputDir: "assets/baidu-ai-pm/",
  achievements: [...] }
```
...
2. **匹配**：说「从素材库匹配这个 JD」→ 解析 JD → 匹配经历（有历史改写则加分）→ 排序 → 你确认 → 改写 → 填模板 → **改写版自动存回素材库**

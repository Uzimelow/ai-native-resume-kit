...
- **修复**：SKILL.md 工作流 5 改为 compact 单页设计：3 段式（15%-60%-25%）+ 底部两栏（数据溯源表 + 设计说明），字数 ~150，适配 BOSS直聘单条消息上限。一屏不滚动
- **设计风格**：与简历主题统一（navy+暖白+宋体），紧凑密度
- **影响**：self-intro 从 800px 竖向篇幅压缩到单页可见，信息密度提升
